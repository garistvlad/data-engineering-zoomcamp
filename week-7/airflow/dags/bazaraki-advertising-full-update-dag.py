...
with DAG(
    ...
    t5 = DockerOperator(
        task_id='parser-process-attributes-full',
        command=f"""
    ...
    t7 = LocalFilesystemToGCSOperator(
        ...
        src=f"{Path(PROCESSED_DATA_DIR, 'features-full')}/*.parquet",
        dst="parsed-from-bazaraki/features-full/",
        bucket="bazaraki-bucket",
        gcp_conn_id=GCP_CONNECTION_ID,
        gzip=False,
    )
    ...
    t10 = BigQueryTableExistenceSensor(
        task_id="bq-check-ads-table-created",
    ...
    for rubric_id in [678, 3528, 2405, 141, 2790, 3303, 142, 681, 3529, 2408, 3530, 2191, 434, 3531]:
        table_name = f"bazaraki.external_features_rubric_{rubric_id}_full"
        t12.append(
            BigQueryExecuteQueryOperator(
                task_id=f"bq-create-{table_name}",
                sql=f"""
                ...
                OPTIONS (
                    format="PARQUET",
                    uris=[
                    'gs://bazaraki-bucket/parsed-from-bazaraki/features-full/rubric_{rubric_id}.parquet'
                    ]
                )
                """,
                use_legacy_sql=False,
